...
object Lists:
  ...
  def max(xs: List[Int]): Int =
    if xs.isEmpty then throw new java.util.NoSuchElementException("empty list")
    else maxRecursive(xs.head, xs.tail)

  def maxRecursive(x: Int, ys: List[Int]): Int =
    if ys.isEmpty then x
    else if x > ys.head then maxRecursive(x, ys.tail)
    else maxRecursive(ys.head, ys.tail)
